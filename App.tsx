      ...
      <main className="flex-1 w-full max-w-7xl flex flex-col items-center pt-[180px] pb-12 px-6 md:px-12 relative z-10">
        ...
        {!showCommittees ? (
          <div className="w-full flex flex-col items-center">
              ...
              {MACRO_AREAS.map((area) => {
                ...
                return (
                  <div 
                    key={area.id}
                    className={`
                      relative flex flex-col bg-white rounded-[32px] overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.03)]
                      transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] border border-slate-100
                      ${isExpanded ? 'scale-[1.03] shadow-[0_35px_80px_rgba(0,0,0,0.08)] ring-1 ring-[#470082]/5' : 'hover:-translate-y-2 hover:shadow-[0_15px_45px_rgba(0,0,0,0.05)]'}
                    `}
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div 
                      onClick={() => toggleMacro(area.id)}
                      className="p-10 cursor-pointer relative overflow-hidden flex items-center justify-between min-h-[160px]"
                      style={{ backgroundColor: area.color }}
                    >
                      <div className="absolute inset-0 bg-gradient-to-br from-white/15 to-black/5 pointer-events-none" />
                      <h3 className="text-xl font-black text-white leading-tight z-10 tracking-tight pr-4 drop-shadow-sm self-start">
                        {area.name}
                      </h3>
                      
                      <div className={`
                        w-12 h-20 rounded-[24px] bg-white/20 backdrop-blur-md flex items-center justify-center 
                        transition-all duration-500 z-10 border border-white/40 shadow-sm self-center
                        ${isExpanded ? 'rotate-180' : ''}
                      `}>
                         <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                      </div>
                    </div>
                    ...
                    <div className="py-4 px-8 bg-white flex items-center justify-center border-t border-slate-50 cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => toggleMacro(area.id)}>
            ...
            {/* Connection Rail SVG - Increased h-40 for better visual breathing */}
            <div className="w-full h-40 flex justify-center items-center pointer-events-none">
               <svg width="100%" height="100%" className="overflow-visible">
                  <line x1="16.6%" y1="0" x2="16.6%" y2="80" stroke="#CBD5E1" strokeWidth="2" strokeDasharray="6 6" />
                  <line x1="50%" y1="0" x2="50%" y2="80" stroke="#CBD5E1" strokeWidth="2" strokeDasharray="6 6" />
                  <line x1="83.3%" y1="0" x2="83.3%" y2="80" stroke="#CBD5E1" strokeWidth="2" strokeDasharray="6 6" />
                  <line x1="16.6%" y1="80" x2="83.3%" y2="80" stroke="#CBD5E1" strokeWidth="2" strokeDasharray="6 6" />
                  <line x1="50%" y1="80" x2="50%" y2="160" stroke="#CBD5E1" strokeWidth="2" strokeDasharray="6 6" />
               </svg>
            </div>

            {/* Strategic Areas Grid */}
            <div className="w-full grid grid-cols-2 md:grid-cols-6 gap-6 relative">
              {STRATEGIC_AREAS.map((area) => (
                <div key={area.id} className="relative h-full">
                  <button 
                    ...
                    className={`
                      w-full h-full min-h-[140px] group bg-white rounded-[28px] p-6 shadow-[0_6px_20px_rgba(0,0,0,0.02)] border border-slate-100 transition-all duration-500
                      hover:shadow-[0_20px_50px_rgba(0,0,0,0.06)] hover:-translate-y-2 flex flex-col items-center justify-center text-center relative z-10
                      ${activeStrategicPopupId === area.id ? 'border-[#470082]/30 ring-2 ring-[#470082]/5' : ''}
                    `}
                  >
                    <div className={`w-14 h-14 rounded-[20px] bg-slate-50 flex items-center justify-center mb-5 transition-all duration-500 text-slate-400 group-hover:text-[#470082] group-hover:bg-[#470082]/5 group-hover:scale-110 icon-animate`}>
                      {getStrategicIcon(area.id)}
                    </div>
                    {/* text-[19px] (+3pts from 16px base) and font-bold as requested */}
                    <h5 className="text-[19px] font-bold text-[#1D1D1F] leading-tight tracking-tight group-hover:text-[#470082] transition-colors">
                      {area.name}
                    </h5>
                  </button>
                  
                  {/* Strategic Pop-up Balloon */}
                  {activeStrategicPopupId === area.id && (
                    <div className="absolute bottom-[calc(100%+1.5rem)] left-1/2 -translate-x-1/2 w-[300px] bg-[#470082] text-white p-8 rounded-[40px] shadow-[0_30px_70px_rgba(71,0,130,0.3)] z-50 animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-500">
                      <p className="text-sm font-medium leading-relaxed italic opacity-90">
                        "{area.description}"
                      </p>
                      <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-[#470082] rotate-45" />
                    </div>
                  )}
                </div>
              ))}
